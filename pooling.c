...
#pragma omp parallel for collapse(2)
	for (int i = 0; i < height; i += 2)
	{
		for (int j = 0; j < width; j += 2)
		{
			...
			int maxR = max(max(image[main], image[down]),
					max(image[right], image[diagonal]));
			int maxG = max(max(image[main + 1], image[down + 1]),
					max(image[right + 1], image[diagonal + 1]));
			int maxB = max(max(image[main + 2], image[down + 2]),
					max(image[right + 2], image[diagonal + 2]));
